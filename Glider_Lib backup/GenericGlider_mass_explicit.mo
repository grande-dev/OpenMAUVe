...
model GenericGlider_mass_explicit "Main glider modelling layer"
  inner Modelica.Mechanics.MultiBody.World world(g = g, n(each displayUnit = "1") = {0, 0, 1}) annotation(
    Placement(visible = true, transformation(extent = {{-14, 44}, {6, 64}}, rotation = 0)));
  ...
  Modelica.SIunits.Mass m_0 "net mass";
  ...
  Modelica.Mechanics.MultiBody.Parts.Body hull(I_11 = I_11, I_22 = I_22, I_33 = I_33, angles_fixed = true, angles_start(displayUnit = "rad"), animation = false, m = m_h, r_0(fixed = true, start = r_0), r_CM = r_CM_hull, sequence_start = {3, 2, 1}, v_0(fixed = true, start = v_0), w_0_fixed = true, w_0_start = w_0) annotation(
  ...
  Glider_Lib.Parts.AddedMassForcesTorques addedMassForcesTorques(K_pdot = K_pdot, M_qdot = M_qdot, M_wdot = M_wdot, N_rdot = N_rdot, N_vdot = N_vdot, X_udot = X_udot, Y_rdot = Y_rdot, Y_vdot = Y_vdot, Z_qdot = Z_qdot, Z_wdot = Z_wdot) annotation(
    Placement(visible = true, transformation(origin = {-63, -60}, extent = {{-69, -18}, {69, 18}}, rotation = 0)));
