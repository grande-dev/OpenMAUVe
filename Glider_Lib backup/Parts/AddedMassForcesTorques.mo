...
model AddedMassForcesTorques "Forces and torques caused by the apparent mass"
  ...
  Modelica.Blocks.Sources.RealExpression added_mass_x(y = -(X_udot * absoluteSensor.a[1] + Z_wdot * absoluteSensor.v[3] * absoluteSensor.w[2] + Z_qdot * absoluteSensor.w[2] ^ 2 - Y_vdot * absoluteSensor.v[2] * absoluteSensor.w[3] - Y_rdot * absoluteSensor.w[3] ^ 2)) annotation(
    Placement(visible = true, transformation(origin = {-144, 99}, extent = {{-54, -27}, {54, 27}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression added_mass_y(y = -(Y_vdot * absoluteSensor.a[2] + Y_rdot * absoluteSensor.z[3] + X_udot * absoluteSensor.v[1] * absoluteSensor.w[3] - Z_wdot * absoluteSensor.v[3] * absoluteSensor.w[1] - Z_qdot * absoluteSensor.w[1] * absoluteSensor.w[2])) annotation(
    Placement(visible = true, transformation(origin = {-144, 65}, extent = {{-54, -27}, {54, 27}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression added_mass_z(y = -(Z_wdot * absoluteSensor.a[3] + Z_qdot * absoluteSensor.z[2] - X_udot * absoluteSensor.v[1] * absoluteSensor.w[2] + Y_vdot * absoluteSensor.v[2] * absoluteSensor.w[1] + Y_rdot * absoluteSensor.w[3] * absoluteSensor.w[1])) annotation(
  ...
  Modelica.Blocks.Sources.RealExpression added_inertia_y(y = -(Z_qdot * (absoluteSensor.a[3] - absoluteSensor.v[1] * absoluteSensor.w[2]) + M_qdot * absoluteSensor.z[2] - (Z_wdot - X_udot) * absoluteSensor.v[3] * absoluteSensor.v[1] - Y_rdot * absoluteSensor.v[2] * absoluteSensor.w[1] + (K_pdot - N_rdot) * absoluteSensor.w[3] * absoluteSensor.w[1]))  annotation(
    Placement(visible = true, transformation(origin = {-150, -69}, extent = {{-54, -27}, {54, 27}}, rotation = 0)));
  Modelica.Blocks.Sources.RealExpression added_inertia_z(y = -(Y_rdot * absoluteSensor.a[2] + N_rdot * absoluteSensor.z[3] - (X_udot - Y_vdot) * absoluteSensor.v[1] * absoluteSensor.v[2] + Y_rdot * absoluteSensor.v[1] * absoluteSensor.w[3] + Z_qdot * absoluteSensor.v[3] * absoluteSensor.w[1] - (K_pdot - M_qdot) * absoluteSensor.w[1] * absoluteSensor.w[2])) annotation(
...
equation
  ...
  connect(added_mass_x.y, force.force[1]) annotation(
    Line(points = {{-84, 100}, {-58, 100}, {-58, 66}, {-40, 66}}, color = {0, 0, 127}));
  connect(added_mass_y.y, force.force[2]) annotation(
    Line(points = {{-84, 66}, {-40, 66}}, color = {0, 0, 127}));
  connect(added_mass_z.y, force.force[3]) annotation(
    Line(points = {{-84, 28}, {-58, 28}, {-58, 66}, {-40, 66}}, color = {0, 0, 127}));
  connect(added_inertia_x.y, torque.torque[1]) annotation(
    Line(points = {{-92, -32}, {-50, -32}, {-50, -68}, {-18, -68}}, color = {0, 0, 127}));
  connect(added_inertia_y.y, torque.torque[2]) annotation(
    Line(points = {{-90, -68}, {-18, -68}}, color = {0, 0, 127}));
  connect(added_inertia_z.y, torque.torque[3]) annotation(
    Line(points = {{-90, -106}, {-50, -106}, {-50, -68}, {-18, -68}}, color = {0, 0, 127}));
  annotation(
