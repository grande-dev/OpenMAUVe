...
model BodyVariableMass
  ...
  SI.Velocity v_0[3](start={0,0,0}, each stateSelect=if
        enforceStates then StateSelect.always else StateSelect.avoid)
  ...
  final parameter SI.Inertia I[3,3]=[I_11,I_21,I_31; I_21,I_22,
  ...
  SI.AngularAcceleration z_a[3](start=Frames.resolve2(R_start,
...
protected
  ...
  SI.Angle phi[3](start=phi_start, each stateSelect=if
  ...
  SI.AngularVelocity phi_d[3](each stateSelect=if enforceStates
