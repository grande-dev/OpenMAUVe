...
model TestSeawing "This model tests a Seawing glider as described in #99."
  ...
  Modelica.Blocks.Sources.Ramp VBD_input(height = 0.3, duration = 0, startTime = 80)  annotation(
    Placement(transformation(origin = {-78, 38}, extent = {{-6, -6}, {6, 6}})));
  Modelica.Blocks.Sources.Ramp m_s_input(height = 0.4216 - 0.4196, duration = 5, startTime = 30)  annotation(
  ...
  Modelica.Blocks.Sources.Ramp m_r_input(duration = 10, height = 45.0, startTime = 150) annotation(
...
equation
  connect(environmental_currents.y, gliderSeawing.env_current_speed) annotation(
    Line(points = {{-48, 86}, {44, 86}, {44, 60}}, color = {0, 0, 127}, thickness = 0.5));
