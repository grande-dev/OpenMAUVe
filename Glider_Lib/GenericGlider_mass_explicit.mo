...
model GenericGlider_mass_explicit "Main glider modelling layer"
  inner Modelica.Mechanics.MultiBody.World world(g = g, n(each displayUnit = "1") = {0, 0, 1} ) annotation(Placement(visible = true, transformation(extent = {{-14, 44}, {6, 64}}, rotation = 0)));
  ...
  parameter Modelica.SIunits.Position r_p[3] = {0.0, 0.0, 0.0} "Shifting mass position wrt to COB";
  parameter Modelica.SIunits.Position r_batt_enclosure[3] = {0.0, 0.0, 0.0} "Position of battery enclosure";    
  parameter Modelica.SIunits.Position r_HV_batt[3] = {0.0, 0.0, 0.0} "Distance from the battery enclosure to the HV battery"; 

  parameter Modelica.SIunits.Inertia I_11 = 0.6 "(1,1) element of inertia tensor of hull";
  parameter Modelica.SIunits.Inertia I_22 = 15.27 "(2,2) element of inertia tensor of hull";
  parameter Modelica.SIunits.Inertia I_33 = 15.32 "(3,3) element of inertia tensor of hull";
  parameter Modelica.SIunits.Mass m_h = 54.28 "Mass of rigid body (hull)";
  parameter Modelica.SIunits.Mass m_r = 11.0 "Mass of rolling mass";
  parameter Modelica.SIunits.Mass m_w = 65.28 "Mass of water displaced by the vehicle";
  parameter Modelica.SIunits.Mass m_battery_enclosure = 0.0 "Mass of battery enclosure";
  parameter Modelica.SIunits.Mass m_HV_battery = 0.0 "Mass of HV battery";
  parameter Modelica.SIunits.Volume nabla_0 = 0.0 "Neutral vehicle volume";


  parameter Modelica.SIunits.Inertia I_11_r = 0.2 "(1,1) element of inertia tensor of rolling mass";
  parameter Modelica.SIunits.Inertia I_22_r = 10.16 "(2,2) element of inertia tensor of rolling mass";
  parameter Modelica.SIunits.Inertia I_33_r = 0.17 "(3,3) element of inertia tensor of rolling mass";
  ...
  parameter Real K_alpha(unit="kg/(m.rad)") = 440.99
    "(also K_L) lift coefficient (related to angle of attack)";
  parameter Real K_MR(unit="kg/rad") = -58.27
    "viscous moment coefficient around x-axis (related to angle of attack)";
  parameter Real K_p(unit="kg.s/rad") = -19.83
    "viscous moment coefficient around x-axis (related to angle of attack)";
  parameter Real K_M0(unit="kg") = 0.28
  ...
  Glider_Lib.Parts.Actuators actuators(I_r_x = I_11_r, I_r_y = I_22_r, I_r_z = I_33_r, m_HV_battery = m_HV_battery, m_battery_enclosure = m_battery_enclosure,m_r = m_r, r_HV_batt = r_HV_batt, r_b = r_b, r_batt_enclosure = r_batt_enclosure, r_p = r_p, r_r = r_r) annotation(Placement(visible = true, transformation(extent = {{-62, 78}, {-32, 98}}, rotation = 0)));
  Parts.HydrodynamicForcesTorques hydrodynamicForcesTorques(K_D = K_D, K_D0 = K_D0, K_L0 = K_L0, K_M = K_M, K_M0 = K_M0, K_MR = K_MR, K_MY = K_MY, K_Ome_1_1 = K_Ome_1_1, K_Ome_1_2 = K_Ome_1_2, K_Ome_2_1 = K_Ome_2_1, K_Ome_2_2 = K_Ome_2_2, K_Ome_3_1 = K_Ome_3_1, K_Ome_3_2 = K_Ome_3_2, K_alpha = K_alpha, K_beta = K_beta, K_p = K_p, K_q = K_q, K_r = K_r)  if true annotation(Placement(transformation(extent = {{-88, -10}, {-28, 10}})));
  Modelica.Blocks.Interfaces.RealInput in_pos_m_r(unit = "rad") annotation(Placement(visible = true, transformation(extent = {{-124, 78}, {-92, 110}}, rotation = 0), iconTransformation(origin = {-100, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput in_pos_m_s(unit = "rad") annotation(Placement(visible = true, transformation(extent = {{-124, 52}, {-92, 84}}, rotation = 0), iconTransformation(origin = {-103, 75}, extent = {{-9, -9}, {9, 9}}, rotation = 0)));
  parameter Modelica.SIunits.Position r_0[3] = {0, 0, 0} "Position vector from origin of world frame to origin of hull";
  parameter Modelica.SIunits.Velocity v_0[3] = {0.0001, 0, 0.00005} "Absolute velocity of frame_a, resolved in world frame (= der(r_0))";
  ...
  Modelica.Blocks.Interfaces.RealOutput out_lin_vel_u annotation(
    Placement(visible = true, transformation(origin = {110, 70}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, 74}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealOutput out_lin_vel_w annotation(
    Placement(visible = true, transformation(origin = {110, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, -36}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
equation
  ...
  connect(hull.frame_a, shape_hull.frame_a) annotation(
    Line(points = {{82, -30}, {61, -30}, {61, -50}, {84, -50}}));
  connect(hydrodynamicForcesTorques.frame_b, hull.frame_a) annotation(
    Line(points = {{-28, 0}, {8, 0}, {8, -30}, {82, -30}}, color = {95, 95, 95}));
  connect(addedMassForcesTorques.frame_b, hull.frame_a) annotation(
    Line(points = {{-10, -60}, {8, -60}, {8, -30}, {82, -30}}, color = {95, 95, 95}));
  connect(absoluteAngles.frame_a, hull.frame_a) annotation(
