...
model World
  ...
  parameter Modelica.Units.SI.Distance axisLength=nominalLength/2
  ...
  parameter Modelica.Units.SI.Distance axisDiameter=axisLength/defaultFrameDiameterFraction
  ...
  parameter Modelica.Units.SI.Position gravityArrowTail[3]={0,0,0}
  ...
  parameter Modelica.Units.SI.Length gravityArrowLength=axisLength/2 "Length of gravity arrow"
    annotation (Dialog(tab="Animation", group=
          "if animateGravity = true and gravityType = UniformGravity",
          enable=enableAnimation and animateGravity and gravityType == GravityTypes.UniformGravity));
  parameter Modelica.Units.SI.Diameter gravityArrowDiameter=gravityArrowLength/
  ...
  input Types.Color gravityArrowColor={0,230,0} "Color of gravity arrow"
    annotation (Dialog(colorSelector=true, tab="Animation", group=
          "if animateGravity = true and gravityType = UniformGravity",
          enable=enableAnimation and animateGravity and gravityType == GravityTypes.UniformGravity));
  parameter Modelica.Units.SI.Diameter gravitySphereDiameter=12742000
  ...
  parameter Modelica.Mechanics.MultiBody.Types.Axis groundAxis_u=if abs(n[1])>=0.99 then {0,1,0} else {1,0,0}
  ...
  parameter Modelica.Units.SI.Length groundLength_u=2 "Length of ground plane along groundAxis_u"
    annotation (Dialog(
      tab="Animation", group="if animateGround = true and gravityType = UniformGravity",
      enable=enableAnimation and animateGround and gravityType == GravityTypes.UniformGravity));
  parameter Modelica.Units.SI.Length groundLength_v=groundLength_u "Length of ground plane perpendicular to groundAxis_u"
    annotation (Dialog(
      tab="Animation", group="if animateGround = true and gravityType = UniformGravity",
      enable=enableAnimation and animateGround and gravityType == GravityTypes.UniformGravity));
  ...
  parameter Modelica.Units.SI.Length nominalLength=1 "Nominal length of multi-body system"
    annotation (Dialog(tab="Defaults"));
  parameter Modelica.Units.SI.Length defaultAxisLength=nominalLength/5
    "Default for length of a frame axis (but not world frame)"
    annotation (Dialog(tab="Defaults"));
  parameter Modelica.Units.SI.Length defaultJointLength=nominalLength/10
    "Default for the fixed length of a shape representing a joint"
    annotation (Dialog(tab="Defaults"));
  parameter Modelica.Units.SI.Length defaultJointWidth=nominalLength/20
    "Default for the fixed width of a shape representing a joint"
    annotation (Dialog(tab="Defaults"));
  parameter Modelica.Units.SI.Length defaultForceLength=nominalLength/10
    "Default for the fixed length of a shape representing a force (e.g., damper)"
    annotation (Dialog(tab="Defaults"));
  parameter Modelica.Units.SI.Length defaultForceWidth=nominalLength/20
    "Default for the fixed width of a shape representing a force (e.g., spring, bushing)"
    annotation (Dialog(tab="Defaults"));
  parameter Modelica.Units.SI.Length defaultBodyDiameter=nominalLength/9
  ...
  parameter Real defaultWidthFraction=20
    ...
    annotation (Dialog(tab="Defaults"));
  parameter Modelica.Units.SI.Length defaultArrowDiameter=nominalLength/40
  ...
  replaceable function gravityAcceleration =
       Glider_Lib.Kinematics.standardGravityAcceleration (
...
protected
  ...
  parameter Modelica.Units.SI.Length headLength=min(axisLength, axisDiameter*Types.Defaults.
      FrameHeadLengthFraction);
  parameter Modelica.Units.SI.Length headWidth=axisDiameter*Types.Defaults.
      FrameHeadWidthFraction;
  parameter Modelica.Units.SI.Length lineLength=max(0, axisLength - headLength);
  parameter Modelica.Units.SI.Length lineWidth=axisDiameter;

  // Parameters to define axes labels
  parameter Modelica.Units.SI.Length scaledLabel=Modelica.Mechanics.MultiBody.Types.Defaults.FrameLabelHeightFraction*
      axisDiameter;
  parameter Modelica.Units.SI.Length labelStart=1.05*axisLength;
  ...
  parameter Modelica.Units.SI.Length gravityHeadLength=min(gravityArrowLength,
      gravityArrowDiameter*Types.Defaults.ArrowHeadLengthFraction);
  parameter Modelica.Units.SI.Length gravityHeadWidth=gravityArrowDiameter*Types.Defaults.ArrowHeadWidthFraction;
  parameter Modelica.Units.SI.Length gravityLineLength=max(0, gravityArrowLength - gravityHeadLength);
