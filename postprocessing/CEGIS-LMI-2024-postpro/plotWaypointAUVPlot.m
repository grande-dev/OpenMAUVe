function [fig_handle] = plotWaypointAUVPlot(x_wp, y_wp, z_wp, radius_waypoint, gamma, alpha_value, rgb_waypoints, is_origin)
    ...
    for iS=1:num_spheres
        ...
        if is_origin
            ...
            S(iS) = scatter3(x_wp(iS),y_wp(iS),z_wp(iS), radius_waypoint, 'MarkerFaceColor', rgb_waypoints, 'HandleVisibility', 'on', ...
                'MarkerFaceAlpha', alpha_value, 'MarkerEdgeAlpha', alpha_value, 'DisplayName', 'origin');
            fig_handle = S(iS);
            ...
            t = text(x_wp(iS)+gamma,y_wp(iS)-gamma,z_wp(iS), ...
                num2str(0), ... % Incremental number as text
            ...
            S(iS) = scatter3(x_wp(iS),y_wp(iS),z_wp(iS), radius_waypoint, 'MarkerFaceColor', rgb_waypoints, 'HandleVisibility', 'on', ...
                'MarkerFaceAlpha', alpha_value, 'MarkerEdgeAlpha', alpha_value,'DisplayName', 'monopile target');
