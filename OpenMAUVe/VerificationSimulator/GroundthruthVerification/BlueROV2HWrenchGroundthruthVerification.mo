...
model BlueROV2HWrenchGroundthruthVerification "Compare BlueROV2H wrench-driven simulator states against groundtruth dataset."
  ...
  parameter Real maxEAAbsErr = 5.0 "Max absolute Euler Angle error per axis [deg]";
  ...
  Modelica.Blocks.Interfaces.RealInput sim_pos[3] annotation(
    Placement(transformation(origin = {-186, 130}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-184, 180}, extent = {{-20, -20}, {20, 20}})));
  Modelica.Blocks.Interfaces.RealInput sim_EA[3] annotation(
    Placement(transformation(origin = {-186, 98}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-186, 128}, extent = {{-20, -20}, {20, 20}})));
  Modelica.Blocks.Interfaces.RealInput sim_vel_body[3] annotation(
    Placement(transformation(origin = {-186, 60}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-186, 70}, extent = {{-20, -20}, {20, 20}})));
  Modelica.Blocks.Interfaces.RealInput sim_ang_vel_body[3] annotation(
    Placement(transformation(origin = {-186, 28}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-186, 10}, extent = {{-20, -20}, {20, 20}})));
  ...
  Modelica.Blocks.Interfaces.RealInput gt_EA[3] annotation(
    Placement(transformation(origin = {-186, -76}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-186, -102}, extent = {{-20, -20}, {20, 20}})));
  Modelica.Blocks.Interfaces.RealInput gt_vel_body[3] annotation(
    Placement(transformation(origin = {-186, -110}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-186, -154}, extent = {{-20, -20}, {20, 20}})));
  Modelica.Blocks.Interfaces.RealInput gt_ang_vel_body[3] annotation(
    Placement(transformation(origin = {-186, -146}, extent = {{-20, -20}, {20, 20}}), iconTransformation(origin = {-184, -214}, extent = {{-20, -20}, {20, 20}})));
...
equation
  for i in 1:3 loop
    ...
    EAAbsErr[i] = abs(sim_EA[i] - gt_EA[i]);
  ...
  EAMaxAbsErr = max({EAAbsErr[1], EAAbsErr[2], EAAbsErr[3]});
  ...
  testPassed = if time >= checkTimeInit and time <= checkTimeFinal then
      if (posMaxAbsErr <= maxPosAbsErr and EAMaxAbsErr <= maxEAAbsErr and velMaxAbsErr <= maxVelAbsErr and angVelMaxAbsErr <= maxAngVelAbsErr) then 1.0 else 0.0
    else -1.0;
end BlueROV2HWrenchGroundthruthVerification;