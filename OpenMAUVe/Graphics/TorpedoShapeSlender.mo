...
model TorpedoShapeSlender "A shape of a cylidrical hull with a cylindrical nose cone."
  ...
  Modelica.Mechanics.MultiBody.Visualizers.FixedShape shapeHull(height = hull_diameter, length = (hull_length/2 + hull_length/2*percentage_slender - hull_diameter/2), shapeType = "cylinder", width = hull_diameter, r_shape = {-(hull_length*percentage_slender)/2, 0.0, 0.0}, color = hull_color, animation = show_shapes) annotation(
